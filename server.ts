...
const getAllowedCorsOrigins = () => {
  const configuredOrigins = [
    process.env.APP_URL,
    "https://whatsapp.shivammadaan.com",
    "http://localhost:3000",
    "http://127.0.0.1:3000",
    "http://localhost:5173",
    "http://127.0.0.1:5173"
...
console.log(`${API_URL}/meta/webhook`);
  ...
  app.post("/api/meta/leads/connect-page", async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({
        error: error.response?.data?.error?.message || "Unable to connect the selected Meta Page to WhatsApp Business CRM.",
      });
    }
  });
  ...
  app.all("/api/wa/*", async (req, res) => {
    ...
    if (!accessToken) {
      console.warn("WhatsApp Proxy: No workspace access token provided");
      return res.status(401).json({ error: "No WhatsApp access token provided for this workspace" });
    }
